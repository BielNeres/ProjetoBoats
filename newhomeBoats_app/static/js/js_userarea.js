...
function MarkMessageReadInDB(msg_from) {
    $.ajax({
        type: 'POST',
        url: "../mark-msg-read",
        data: {
        ...
        success:function(data){
            console.log("success");
            let no_of_unread = parseInt($('#notif_'+msg_from).html());
            $('#notif_'+msg_from).css("display","none");
            $('.notif_no').each(function () {
                let notif_no = parseInt($(this).html());
                let new_notif_no = notif_no - no_of_unread;
                if (new_notif_no > 0) {
                    $(this).html(new_notif_no)
                } else {
...
function OpenContactChat(objId) {
    ...
    MarkMessageReadInDB(contact_id);
    ...
    $('#msg_content_'+contact_id+' div div').each(function () {
        ...
        if (obj.attr('msg_type') === 'from') {
           ...
           obj.parent().find('.ua_msg_date').addClass('message-buble-date-right');
        } else if (obj.attr('msg_type') === 'to') {
           obj.addClass('message-buble-left')
               .parent().find('.img-left')
               .removeClass('hideOnly')
               .find('img').attr('src', img_src);
           obj.parent().find('.ua_msg_date').addClass('message-buble-date-left');
        }
    });
...
function DragBoatPhoto(ev) {
    ...
    let imagem_id = ev.target.id;

    // get dragged image src
    let dragged_srv = $('#'+imagem_id).attr("src");
    ev.dataTransfer.setData("src", dragged_srv);

    // get dragged image input value
    let inp_val = $('#'+(imagem_id.replace("img","id"))).val();
    ev.dataTransfer.setData("val", inp_val);

    // get dragged image id
    ev.dataTransfer.setData("id", imagem_id);
}

function DropBoatPhoto(ev) {
    ev.preventDefault();

    // setup main picture square
    $('#edt_mainpic').removeClass("hideOnly");
    $('#div_mainpic').css("border", "none");

    // set dragged image to main picture square
    let data = ev.dataTransfer.getData("src");
    $('#edt_mainpic').attr("src", data);

    // set dragged image input value to main picture input value
    $('#selected_main_photo').val(ev.dataTransfer.getData("val"));

    // remove main picture class design from the current main picture
    $('.edit_img .selected_pic').each(function () {
       $(this).removeClass("selected_pic");
    });

    // set main picture class design to the new main picture
    let img_id = (ev.dataTransfer.getData("id")).replace("img","view");
    $('#'+img_id).addClass("selected_pic");
    $('#main_photo').remove();
    $('#'+img_id).parent().append("<p class=\"fontSizeZeroEight\" style=\"text-align: center;padding-top: 5px;\" id=\"main_photo\">DESTAQUE</p>");
}
...
function DeletePhoto(obj_id) {
    ...
    if ($('#selected_main_photo').val() === $('#' + obj_id + "_id").val()) {
        $('#selected_main_photo').val("");
        $('#edt_mainpic').attr("src","").addClass("hideOnly");
        $('#drag_drop_message').removeClass("hideOnly");
        $('#' + obj_id + "_view").removeClass("selected_pic");
        $('#main_photo').remove();
    }

    // remove image input value
    $('#' + obj_id + "_id").val("");

    // remove image view from photo modal
    $('#' + obj_id + "_img").attr("src","");

    // remove image view from boat update form
    $('#' + obj_id + "_view").attr("src","");
        ...
        success: function (data) {
            ...
            if (data.images !== null && data.images !== "") {
                ...
                for (i = 0; i < boat_images.length - 1; i++) {
                    if ($('#edt_inp_pic_' + (i + 1) + "_view").length) {
                        boat_images[i] = "https://bucket-principal.s3.amazonaws.com/static/" + boat_images[i];
                        $('#edt_inp_pic_' + (i + 1) + "_view").attr("src", boat_images[i]);
                    }
                    $('#edt_inp_pic_' + (i + 1) + "_img").attr("src", boat_images[i]);
                    $('#edt_inp_pic_' + (i + 1) + "_id").val(boat_images[i].replace("https://bucket-principal.s3.amazonaws.com/static/",""));
...
function SaveBlockingDates(date_chk) {
    let barco = $('#edt_updated_boat').val();
    let date_array_from = ($('#block_date_from').val()).split("/");
    let date_array_to = ($('#block_date_to').val()).split("/");
    //let date_chk = $('#chk_block_date_from').is(':checked');

    if (date_array_from.length !== 3) {
        return;
    }

    if (date_array_to === "") {
        date_array_to = date_array_from;
    }
    ...
    $.ajax({
        ...
        data: {
            "barco": barco,
            "date_from": date_array_from[2]+"-"+date_array_from[1]+"-"+date_array_from[0],
            "date_to": date_array_to[2]+"-"+date_array_to[1]+"-"+date_array_to[0],
...
function SetDateAvailability() {
    ...
    if ($('#chk_block_date_from').is(":checked")) {
        ...
        $('#block_date_to').attr("style", curr_style + "background-color: white !important");
    } else {
        $('#lbl_block_date_from').html("Data indisponível");
        $('#block_date_from').attr("style", curr_style + "background-color: #f1f3f7 !important");
        $('#block_date_to').attr("style", curr_style + "background-color: #f1f3f7 !important");
    }
}

function Userboat_Save(obj, counter, loader_div, form_id) {
    $('#'+loader_div).removeClass("hideOnly");
    $(obj).css("opacity","0.5");
    setTimeout(function () {    //  call a 3s setTimeout when the loop is called
        if (parseInt($('#uploading_img').val()) < 1) {
            if ($('#'+form_id)[0].checkValidity() === true) {
        ...
        if (counter < 20) {            //  if the counter < 10, call the loop function
            Userboat_Save(obj, counter, loader_div, form_id);             //  ..  again which will trigger another
        } else {
            $('#'+loader_div).addClass("hideOnly");
            $(obj).css("opacity","1");
        }                   //  ..  setTimeout()
    }, 1000);
}

function CheckDataToIsValid(DateFromObjID, DateToObjID) {
    let date_from = $('#'+DateFromObjID);
    let date_to = $('#'+DateToObjID);
