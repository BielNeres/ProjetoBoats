...
$(document).ready(function() {
    ...
    // open a previous open modal before login
    if ($('.modal.show').length === 0 && $('.signin_modal').val() !== "" && ($('#user_added').val() !== "true")) {
        let modal_toopen = $('#'+$('.signin_modal').val());
        if (modal_toopen.length === 1) {
            modal_toopen.modal("show");
        }
    }
    if ($('.signin_params').val() !== "") {
        let params = ($('.signin_params').val()).split(";");
    ...
    if ($('input[name=doc_type]:checked', '#SignUpForm').length > 0) {
        ToggleOnyNumbersClass($('input[name=doc_type]:checked', '#SignUpForm').val().toLowerCase(),
            'cpf', 'id_doc_number');
    }
...
function BuildOwnCalendar(month, year, cal_div_id) {
    ...
    for (let i = 0; i < booked_dates.length; i++) {
        ...
        if (booked_date_arr.length === 1) {
            booked_date_arr[1] = booked_date_arr[0];
        }
        if (booked_date_arr.length > 1) {
    ...
    for (let cal_row = 1; cal_row <= (no_weeks-2); cal_row++) {
        ...
        for (cal_cel = 1; cal_cel <= 7; cal_cel++) {
            ...
            // DAY CELL
            week_rows += "<div class='p-0 pointer flex-sm-fill bd-highlight cal_weekd b-right d-flex align-items-center justify-content-center "+blocked+"' onclick='BookShortcutOnMobile("+day+","+month+","+year+","+blocked+")'>";
...
//$('#home_filter_city').on("change", function () {
//    if (ValidateCity("home_filter_submit") === true) {
        //$('#home_filter_submit').trigger('click');
//    }
//});
...
var pureJSCalendar = (function () {
    ...
    function switchMonth(next, month, year) {
        let curr = label.textContent.trim().split(' '), calendar, tempYear = parseInt(curr[1], 10);
        if (month === undefined) {
...
function DisableCalendarDays(field) {
    ...
    if (field.search("from") > -1) {
        $("#" + field).val("");
        $("#" + field.replace("from","to")).val("");
    }

    if (field === "") {
        field = $('.header').attr("data-element");
    }
    ...
    if (field.search("from") > -1) {
        ...
        $("#" + field.replace("from","to")).val("")
    } else if (field.search("to") > -1) {
        curr_date = ($("#"+field.replace("to","from")).val()).split("/");
    ...
    for (i = 0; i < tds.length; i++) {
        ...
        if (!(day==="") && ((year < curr_year) || (year === curr_year && month < curr_month) ||
            (year === curr_year && month === curr_month && day < curr_day)) || is_booked === true && tds[i].innerHTML !== "") {
            is_booked = false;
...
function ForgotPassword() {
    ...
    if (!emailReg.test(email_field.val()) || email_field.val() === "") {
        ...
        results_msg.html("Por favor, entre seu e-mail para enviarmos uma nova senha");
        results_msg.addClass("alert-warning");
        results_msg.removeClass("hideOnly");
        $('#login_btn').val("Enviar senha").attr("onclick","$(this).attr('disabled','disabled').css('opacity','0.6');$('#loading_icon_signin').removeClass('hideOnly');ForgotPassword();");
    } else {
        $.ajax({
           url: '../esqueci-a-senha',
           data: {
            ...
            success: function (data) {
                if (data.status === "ok") {
                    ...
                    results_msg.removeClass("alert-warning");
                    results_msg.addClass("alert-success");
                    results_msg.removeClass("hideOnly");
                    $('#login_btn').attr('disabled',false).css('opacity','1');
                    $('#loading_icon_signin').addClass('hideOnly');
                    $('#login_btn').val("Entrar").attr("onclick","$(this).attr('disabled','disabled').css('opacity','0.6');$('#loading_icon_signin').removeClass('hideOnly');$('#SignInForm').submit();");
                } else {
...
function CloseSMSPopup(obj) {
    ...
    if ($(obj).hasClass("true")) {
        window.location.href = "/meus-anuncios/add";
    }
}

function ValidateCPF(form_id) {

    if ($('select[name=doc_type]', '#'+form_id).children("option:selected").val().toLowerCase() !== "cpf") {
        return;
    }
...
function isElementOnViewport(elem){
    var $elem = $(elem);
...
/*$(window).scroll(function() {
    if (isElementOnViewport('#results_end_mark') === true) {
        let boat_onscreen = "";
        $('.boat_onscreen').each(function () {
            boat_onscreen += $(this).val + ";"
        });
        //alert("Pegou");
    }
});*/
...
function UploadPictureToS3(input_id) {
    ...
    if (file){
        ...
        if (input_id.search("user_picture") > -1) {
            $('#'+input_id+'_loading').removeClass("hideOnly");
            $('#'+input_id+'_img').css("opacity","0.2");

        // existing boat pictures update
        } else if (input_id.search("edt_inp_pic") > -1) {
            $('#'+input_id.substr(0, input_id.length-1) + "loading").removeClass("hideOnly");
            $('#'+input_id+'_img').css("opacity","0.2");
            let uploading_img_counter = parseInt($('#uploading_img').val());
            $('#uploading_img').val(uploading_img_counter + 1);

        // creation of new boat
        } else if (input_id.search("inp_pic") > -1) {
            $('#'+input_id.substr(0, input_id.length-1) + "loading").removeClass("hideOnly");
            $('#'+input_id+'_img').css("opacity","0.2");
            let uploading_img_counter = parseInt($('#uploading_img').val());
            $('#uploading_img').val(uploading_img_counter + 1);
        ...
        // get authorization request from the backend
        getSignedRequest(file, input_id);
    }
}

function getSignedRequest(file, input_id){
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/sign_s3?file_name="+(file.name).replace(/ /g,"_")+"&file_type="+file.type);
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4){
            if(xhr.status === 200){
                ...
                // Upload file to S3
                uploadFile(file, response.data, response.url_prefix, response.url_sufix, input_id);

                // Save image url to database
                if (input_id.search("user_picture") > -1) {
                    updateProfilePicInDb(response.url_sufix);
                }

                // Load image on screen to the user
                loadBoatImgOnScreen(file, input_id+"_img");
                if ($('#'+input_id+"_view").length) {
                    loadBoatImgOnScreen(file, input_id+"_view");
                }

            } else {
                console.log(xhr.responseText);
                alert("Ops, houve um erro no carregamento. Caso o erro persista, por favor, entre em contato através de contato@bnboats.com");
            }
        }
    };
    xhr.send();
...
function loadBoatImgOnScreen(img, objId) {
...
function uploadFile(file, s3Data, url_prefix, url_sufix, input_id){
    ...
    // existing boat pictures update
    if (input_id.search("edt_inp_pic") > -1) {
        let estimated_loading_time = (parseFloat(file.size) / 1000) * 2;
        setTimeout(function () {
            $('#' + input_id.substr(0, input_id.length-1) + "loading").addClass("hideOnly");
            $('#' + input_id + '_img').css("opacity", "1");
            $('#' + input_id + '_id').val(url_sufix);
        }, estimated_loading_time);

    // new boat creation
    } else if (input_id.search("inp_pic") > -1) {
        let estimated_loading_time = (parseFloat(file.size) / 1000);
        setTimeout(function () {
            $('#' + input_id.substr(0, input_id.length-1) + "loading").addClass("hideOnly");
            $('#' + input_id + '_img').css("opacity", "1");
            $('#' + input_id + '_id').val(url_sufix);
    ...
    xhr.onreadystatechange = function() {
        if(xhr.readyState === 4){
            if(xhr.status === 200 || xhr.status === 204){
                if (input_id.search("user_picture") > -1) {
                    $('#' + input_id + '_loading').addClass("hideOnly");
                    $('#' + input_id + '_img').css("opacity", "1");
                }

                // existing boat pictures update
                if (input_id.search("inp_pic") > -1) {
                    uploading_img_counter = parseInt($('#uploading_img').val());
                    $('#uploading_img').val(uploading_img_counter - 1);
                }
...
function updateProfilePicInDb(img_url) {
    $.ajax({
        type: 'POST',
        url: '../trocar-perfil-img',
        data: {
            'img_url': img_url
        },
        complete: function (data) {
...
$('#SignUpForm #id_doc_type').on('change', function () {
    ToggleOnyNumbersClass($('select[name=doc_type]', '#SignUpForm').children("option:selected").val().toLowerCase(),
        'cpf', 'id_doc_number');
    $('#SignUpForm #id_doc_number').val('')
        .removeClass('is-invalid')
        .removeClass('is-valid');
});

$('#SignUpForm #id_doc_number').on('focusout', function () {
    ValidateCPF("SignUpForm");
});


$('#SignUpForm #id_doc_number').on('keyup', function (e) {
    if ($('select[name=doc_type]', '#SignUpForm').children("option:selected").val().toLowerCase() !== "cpf") {
    ...
    if ($('select[name=doc_type]', '#SignUpForm').children("option:selected").val().toLowerCase() === "cnpj") {
