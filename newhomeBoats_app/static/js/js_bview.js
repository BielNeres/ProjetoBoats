...
$(document).ready(function() {
    ...
    $('#modal_login_footer').css("display","none");
...
function BoatImageNextOrPrev(direction, obj_arrow) {
    ...
    let boat_img = parseInt($('#boat_img_' + id).find('img').css('height'));
    if (boat_img > 500) { boat_img = 500; }
    $('#photo_modal_close_div').css("margin-top",((500-boat_img)/2) + 30);
}
...
function Boat_Type_Onchange() {
   ...
   if (btype !== "24") {
      ...
      $('#dates_division').css("display","none");
   } else {
      $('#filter_date_to').attr("disabled", false).addClass("pholder_blue");
      $('#dates_division').css("display","block");
   }
   $('.bprice_p').each(function () {
      $(this).addClass("hideOnly");
   });
   $('#bprice_'+btype).removeClass("hideOnly");
}
...
function CreateBooking() {
    ...
    if ($('#booking_request').hasClass("anonymous")) {
        $('#SignUpBtn').trigger("click");
        $('#modal_alert').removeClass("hideOnly").addClass("alert-warning").html("Por favor, cadastre-se ou entre com suas credenciais para realizar a reserva.");
        $('.signin_modal').val("");
        let booking_type = $('#booking_type_select').val();
        let booking_dt_from = $('#filter_date_from').val();
        let booking_dt_to = $('#filter_date_to').val();
        let booking_pax = $('#booking_pax').val();
        $('.signin_params').val("booking_type_select:"+booking_type+";filter_date_from:"+booking_dt_from+";filter_date_to:"+booking_dt_to+";booking_pax:"+booking_pax+";");
        return;
    }

    if ($('#bview_rd_booking_div').hasClass("Incomplete_User_Profile")) {
        window.location = '../../meu-perfil?next_page=boat_view&param=' + $('#boat_pk').val();
        return;
    }
                        ...
                        "items": [
                            {
                                "sku": payment_id,
                                "name": "Barco",
                                "category": "Status do Pagamento: " + payment_status,
                                "price": payment_total,
                                "quantity": 1
                            }
                        ]
...
function SendMessage() {
    ...
    if (send_btn.hasClass("anonymous")) {
        $('#SignUpBtn').trigger("click");
        $('#modal_alert').removeClass("hideOnly").addClass("alert-warning").html("Por favor, cadastre-se ou entre com suas credenciais para enviar a mensagem ao proprietário.");
        $('.signin_modal').val("modal_captain_chat");
        $('.signin_params').val("captain_chat_text:"+message_text+";");
        return;
    }
...
function BookShortcutOnMobile(day = "", month = "", year = "", day_blocked = "") {
    $('html,body').animate({
        scrollTop: $(".bview_right_div").offset().top
    }, 'slow');
    if ($('#filter_date_from').length === 1 && day_blocked === "") {
       book_date = new Date(year+"/"+month+"/"+day+" 00:00:00");
       if (book_date.toString() !== "Invalid Date") {
           $('#filter_date_from').val(book_date.getDate() + "/" + (book_date.getMonth()+1) + "/" + book_date.getFullYear());
       }
    }
}
...
$('#modal_photos').on('shown.bs.modal', function (e) {
    ...
    let boat_img = parseInt($('#boat_img_1').find('img').css('height'));
    if (boat_img > 500) { boat_img = 500; }
    $('#photo_modal_close_div').css("margin-top",((500-boat_img)/2) + 30);
